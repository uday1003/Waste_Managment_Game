...
FALL_SPEED = 5  # Adjust the fall speed
...
object_type1_images = [pygame.image.load(os.path.join("C:\\Users\\Srimanth\\Downloads\\img", filename)) for filename in os.listdir("C:\\Users\\Srimanth\\Downloads\\img")]
object_type1_images = [pygame.transform.scale(image, (OBJECT_SIZE, OBJECT_SIZE)) for image in object_type1_images]
object_type2_images = [pygame.image.load(os.path.join("C:\\Users\\Srimanth\\Downloads\\img1", filename)) for filename in os.listdir("C:\\Users\\Srimanth\\Downloads\\img1")]
...
instructions_text = [
    "Welcome to the recycle game!",
    "Catch the falling objects with the bucket.",
    "Use the mouse to move the bucket left and right.",
    "Avoid the non recyclable objects to save your lives.",
    "If recycled objects are caught you score will be increased by 1",
...
def adjust_difficulty(score):
    ...
    global m
    if score >= 3:
        FALL_SPEED = 8 + 2*count11 # Increase speed for good performance
        if past_state<3 or past_state == -float('inf'):
            display_message("Great going! Ready for a higher challenge?", 2)
        else:
            count11+=1
        m = [1,0,0]
    elif score == 2:
        if m == [0,0,1]:
            ...
            FALL_SPEED = FALL_SPEED + 2
        elif m == [0,0,0] or m==[0,1,0]:
            FALL_SPEED = FALL_SPEED 

        if (past_state>-1 and past_state<2)  or past_state == -float('inf'):
            display_message("Good! Maintain this and try to imporve", 2)
        elif past_state >= 3:
            display_message("Dropped your level mate common reach the peak", 2)
        m = [0,1,0]
    else:
        if m == [0,0,1] or m == [0,0,0]:
            FALL_SPEED = FALL_SPEED
        elif m == [1,0,0]:
            FALL_SPEED = FALL_SPEED - 4
        elif m == [0,1,0]: 
            FALL_SPEED = FALL_SPEED - 2  # Decrease speed for poor performance
        if (past_state >= 2) or (past_state == -float('inf')):
            display_message("Things have slowed down. Try to score higher!", 2)
        m = [0,0,1]
    past_state = score
while True:
    for event in pygame.event.get():
    ...
    # Display power-up counts
        ...
        if (time.time() - last_difficulty_check_time >= DIFFICULTY_CHECK_INTERVAL) and (remaining_time > 0) and (time.time()-start_time > 9):
            temp1 = score
            diff = temp1 - temp
            adjust_difficulty(diff)
            last_difficulty_check_time = time.time()
            temp = score
        ...
        # Fetch and display the highest score from MongoDB
            highest_score = collection.find_one(sort=[("final_score", pymongo.DESCENDING)])
            if highest_score:
            ...
            for obj in falling_objects:
                if bucket_rect.colliderect(obj['rect']):
                    ...
                    if obj['image'] in object_type1_images:
                        ...
                        # Type 2 object caught, decrease life
                        current_lives -= 1
